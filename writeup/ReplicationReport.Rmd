...
However, as alluded to above, we have reduced the amount of data collected per subject from 5 blocks to 2. Thus we increased our sample size to 30 participants, in order to collect the same total amount of data.
...
```{r}
# subject_aggregated_data_2 = subject_aggregated_data %>% select(subject,rotating,K) %>% spread(rotating,K) %>% mutate(delta_K=`FALSE`-`TRUE`)

# mod_null = lm(data=subject_aggregated_data,K ~ 1)
# summary(mod_null)
# mod_1 = lm(data=subject_aggregated_data,K ~ rotating)
# summary(mod_1)
# anova(mod_null,mod_1)
# 
# mod_2 = lm(data=subject_aggregated_data_2,delta_K ~ 1)
# summary(mod_2)
n_subjects = length(unique(subject_aggregated_data$subject))
# mod_lmer = lmer(data=subject_aggregated_data,K ~ rotating+ (1|subject))
# summary(mod_lmer) 
