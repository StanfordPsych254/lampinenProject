...
subject_aggregated_data = filtered_data %>% group_by(subject,experiment_trial_type,rotating) %>%
  ...
  spread(experiment_trial_type,percent_correct) %>% mutate(K1 = (-b+sqrt(b^2-4*a*(-2*6*(swap-(1-correct)))))/(2*a),K2 = (-b-sqrt(b^2-4*a*(-2*6*(swap-(1-correct)))))/(2*a)) %>% mutate(K = ifelse(K1 <= 4 & K1 >= 0,K1,ifelse(K2 <= 4 & K2 >= 0,K2,NA)))
